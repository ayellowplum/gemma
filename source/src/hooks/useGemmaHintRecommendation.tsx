...
/**
 * Surfaces plugin-install prompts driven by the compatibility
 * `<claude-code-hint />` tag that CLIs/SDKs emit to stderr.
 *
...
import { clearPendingHint, getPendingHintSnapshot, markShownThisSession, subscribeToPendingHint } from '../utils/gemmaHints.js';
  ...
  if ($[0] !== pendingHint || $[1] !== tryResolve) {
    t0 = () => {
      ...
      tryResolve(async () => {
        ...
        if (resolved) {
          logForDebugging(`[useGemmaHintRecommendation] surfacing ${resolved.pluginId} from ${resolved.sourceCommand}`);
