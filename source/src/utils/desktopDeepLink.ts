...
/**
 * Builds a deep link URL for Gemma Desktop to resume a CLI session.
 * Format: gemma://resume?session={sessionId}&cwd={cwd}
 * In dev mode: gemma-dev://resume?session={sessionId}&cwd={cwd}
 */
function buildDesktopDeepLink(sessionId: string): string {
  const protocol = isDevMode() ? 'gemma-dev' : 'gemma'
...
/**
 * Check if Gemma Desktop app is installed.
 * On macOS, checks for /Applications/Gemma.app.
 * On Linux, checks if xdg-open can handle gemma:// protocol.
...
async function isDesktopInstalled(): Promise<boolean> {
  ...
  if (platform === 'darwin') {
    // Check for Gemma.app in /Applications
    return pathExists('/Applications/Gemma.app')
  } else if (platform === 'linux') {
    // Check if xdg-mime can find a handler for gemma://
...
/**
 * Detect the installed Gemma Desktop version.
...
async function getDesktopVersion(): Promise<string | null> {
  ...
  if (platform === 'darwin') {
    const { code, stdout } = await execFileNoThrow('defaults', [
      'read',
      '/Applications/Gemma.app/Contents/Info.plist',
...
/**
 * Build and open a deep link to resume the current session in Gemma Desktop.
...
export async function openCurrentSessionInDesktop(): Promise<{
  ...
  if (!installed) {
    return {
      success: false,
      error:
        'Gemma Desktop is not installed.',
    }
  }
  ...
  if (!opened) {
    return {
      success: false,
      error: 'Failed to open Gemma Desktop. Please try opening it manually.',
      deepLinkUrl,
    }
  }
