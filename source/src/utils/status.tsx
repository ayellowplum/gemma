...
export function buildAPIProviderProperties(): Property[] {
  ...
  if (apiProvider !== 'firstParty') {
    const providerLabel = {
      ...
      gemini: isEnvTruthy(process.env.CLAUDE_CODE_USE_VERTEX)
  ...
  if (apiProvider === 'firstParty') {
    ...
    if (geminiBaseUrl && !isEnvTruthy(process.env.CLAUDE_CODE_USE_VERTEX)) {
      properties.push({
        label: 'Gemini base URL',
        value: geminiBaseUrl
      });
    }
    if (isEnvTruthy(process.env.CLAUDE_CODE_SKIP_GEMINI_AUTH)) {
      properties.push({
        value: 'Gemini auth skipped'
      });
    }
  }
