/**
 * Gemma hints protocol.
 *
 * For compatibility, CLIs and SDKs can emit a self-closing
 ...
 *
 * The wire tag remains `claude-code-hint` so existing integrations keep working.
 */

import { logForDebugging } from './debug.js'
import { createSignal } from './signal.js'

export type GemmaHintType = 'plugin'

export type GemmaHint = {
 ...
 */
export function extractGemmaHints(
  output: string,
  command: string,
): { hints: GemmaHint[]; stripped: string } {
  ...
  const stripped = output.replace(HINT_TAG_RE, rawLine => {
    ...
    if (!SUPPORTED_VERSIONS.has(v)) {
      logForDebugging(
        `[gemmaHints] dropped hint with unsupported v=${attrs.v}`,
      )
      return ''
    }
    if (!type || !SUPPORTED_TYPES.has(type)) {
      logForDebugging(
        `[gemmaHints] dropped hint with unsupported type=${type}`,
      )
      return ''
    }
    if (!value) {
      logForDebugging('[gemmaHints] dropped hint with empty value')
      return ''
    }

    hints.push({ v, type: type as GemmaHintType, value, sourceCommand })
    return ''
  })
...
let pendingHint: GemmaHint | null = null
...
export function setPendingHint(hint: GemmaHint): void {
...
export function getPendingHintSnapshot(): GemmaHint | null {
  return pendingHint
}
...
/** Test-only reset. */
export function _resetGemmaHintStore(): void {
  pendingHint = null
