...
export async function showSetupScreens(root: Root, permissionMode: PermissionMode, allowDangerouslySkipPermissions: boolean, commands?: Command[], claudeInChrome?: boolean, devChannels?: ChannelEntry[]): Promise<boolean> {
  ...
  if (!isEnvTruthy(process.env.CLAUBBIT)) {
    ...
    if (await shouldShowClaudeMdExternalIncludesWarning()) {
      ...
      } = await import('./components/GemmaMdExternalIncludesDialog.js');
      await showSetupDialog(root, done => <GemmaMdExternalIncludesDialog onDone={done} isStandaloneDialog externalIncludes={externalIncludes} />);
    }
  }
  ...
  // Show Chrome onboarding for first-time Gemma-in-Chrome users
  if (claudeInChrome && !getGlobalConfig().hasCompletedClaudeInChromeOnboarding) {
    const {
      GemmaInChromeOnboarding
    } = await import('./components/GemmaInChromeOnboarding.js');
    await showSetupDialog(root, done => <GemmaInChromeOnboarding onDone={done} />);
  }
